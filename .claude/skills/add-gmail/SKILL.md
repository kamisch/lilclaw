---
name: add-gmail
description: Add Gmail integration to LilClaw. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to LilClaw. It can be configured in two modes:
...
> How do you want to use Gmail with LilClaw?
>
> **Option 1: Tool Mode**
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "LilClaw"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "LilClaw Gmail")
>    - Click **Create**
...
```bash
sleep 2 && launchctl list | grep lilclaw  # macOS
...
> - Create a Gmail label (e.g., "LilClaw")
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'LilClaw',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/lilclaw.log | grep -i email
```
...
```bash
tail -f logs/lilclaw.log | grep -E "(email|Email)"
```
