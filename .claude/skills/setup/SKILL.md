---
name: setup
description: Run initial LilClaw setup. Use when user wants to install dependencies, authenticate WhatsApp, register their main channel, or start the background services. Triggers on "setup", "install", "configure lilclaw", or first-time setup requests.
---
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.lilclaw.plist`
- Linux: `systemctl --user stop lilclaw` (or `systemctl stop lilclaw` if root)
...
- macOS: check `launchctl list | grep lilclaw`. If PID=`-` and status non-zero, read `logs/lilclaw.error.log`.
- Linux: check `systemctl --user status lilclaw`.
...
Tell user to test: send a message in their registered chat. Show: `tail -f logs/lilclaw.log`

## Troubleshooting

**Service not starting:** Check `logs/lilclaw.error.log`. Common: wrong Node path (re-run step 10), missing `.env` (step 4), missing auth (step 5).
