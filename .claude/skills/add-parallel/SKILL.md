...
Adds Parallel AI MCP integration to LilClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses LilClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. LilClaw already set up and running
...
Add Parallel AI MCP servers after the lilclaw server:
```typescript
const mcpServers: Record<string, any> = {
  lilclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__lilclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__lilclaw__send_message
   4. Use mcp__lilclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo lilclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep lilclaw  # macOS
...
- Check task runs: `tail -f logs/lilclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.lilclaw` (macOS) or `systemctl --user restart lilclaw` (Linux)
