---
name: x-integration
description: X (Twitter) integration for LilClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** LilClaw v1.0.0. Directory structure may change in future versions.
...
1. **LilClaw is installed and running** - WhatsApp connected, service active
...
npm run build
launchctl kickstart -k gui/$(id -u)/com.lilclaw  # macOS
# Linux: systemctl --user restart lilclaw
# Verify: launchctl list | grep lilclaw (macOS) or systemctl --user status lilclaw (Linux)
```
...
| `LILCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/lilclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into LilClaw, make the following modifications:
...
All paths below are relative to project root (`LILCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep lilclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status lilclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/lilclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/lilclaw.log | tail -20
```
...
docker run lilclaw-agent ls -la /app/src/skills/
```
