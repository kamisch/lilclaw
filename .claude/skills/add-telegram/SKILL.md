...
This skill adds Telegram support to LilClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.lilclaw/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
If `.lilclaw/` directory doesn't exist yet:
...
- Records the application in `.lilclaw/state.yaml`
...
4. Service is running: `launchctl list | grep lilclaw` (macOS) or `systemctl --user status lilclaw` (Linux)
...
- Check bot is started: `tail -f logs/lilclaw.log`
...
# Linux:
# systemctl --user stop lilclaw
# npm run dev
# systemctl --user start lilclaw
```
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.lilclaw` (macOS) or `npm run build && systemctl --user restart lilclaw` (Linux)
