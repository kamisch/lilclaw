...
import { BASE_DIR, CUSTOM_DIR, LILCLAW_DIR } from './constants.js';
import { initLilclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initLilclawDir();
  console.log('Skills system initialized. .lilclaw/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .lilclaw/custom/migration.patch',
      );
    } else {
