...
import { SKILLS_SCHEMA_VERSION, LILCLAW_DIR, STATE_FILE } from './constants.js';
import { AppliedSkill, CustomModification, SkillState } from './types.js';

function getStatePath(): string {
  return path.join(process.cwd(), LILCLAW_DIR, STATE_FILE);
}
...
export function readState(): SkillState {
  ...
  if (!fs.existsSync(statePath)) {
    throw new Error(
      '.lilclaw/state.yaml not found. Run initSkillsSystem() first.',
    );
  }
