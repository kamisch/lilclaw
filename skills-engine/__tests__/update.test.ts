...
import { cleanup, createTempDir, initGitRepo, setupLilclawDir } from './test-helpers.js';
...
describe('update', () => {
  ...
  function writeStateFile(state: Record<string, unknown>): void {
    const statePath = path.join(tmpDir, '.lilclaw', 'state.yaml');
        ...
        custom_modifications: [
          {
            ...
            patch_file: '.lilclaw/custom/001-tweak.patch',
          },
        ],
      });
  ...
  describe('applyUpdate', () => {
    it('rejects when customize session is active', async () => {
      ...
      const customDir = path.join(tmpDir, '.lilclaw', 'custom');
