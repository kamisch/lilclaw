...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lilclaw-test-'));
}

export function setupLilclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.lilclaw', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.lilclaw', 'backup'), { recursive: true });
}
