...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.lilclaw', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.lilclaw', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.lilclaw', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase flattens: base updated to match working tree', async () => {
    ...
    const baseContent = fs.readFileSync(
      path.join(tmpDir, '.lilclaw', 'base', 'src', 'index.ts'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
    // Set up base files
    const baseDir = path.join(tmpDir, '.lilclaw', 'base');
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.lilclaw/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.lilclaw', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.lilclaw', 'base', 'src', 'config.ts'),
      'utf-8',
    );
  ...
  it('rebase clears resolution cache', async () => {
    ...
    const resDir = path.join(tmpDir, '.lilclaw', 'resolutions', 'skill-a+skill-b');
    ...
    const resolutions = fs.readdirSync(
      path.join(tmpDir, '.lilclaw', 'resolutions'),
    );
