...
│  │    • mcp__lilclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── lilclaw.log               # Host stdout
│   └── lilclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.lilclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'lilclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your lilclaw installation
...
LilClaw uses a hierarchical memory system based on CLAUDE.md files.
...
LilClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__lilclaw__schedule_task]
        {
...
### LilClaw MCP (built-in)

The `lilclaw` MCP server is created dynamically per agent call with the current group's context.
...
LilClaw runs as a single macOS launchd service.

### Startup Sequence

When LilClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned LilClaw containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/lilclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/lilclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.lilclaw.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.lilclaw.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.lilclaw.plist

# Check status
launchctl list | grep lilclaw

# View logs
tail -f logs/lilclaw.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep lilclaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; LilClaw auto-starts container runtime but may fail |
...
- `logs/lilclaw.log` - stdout
- `logs/lilclaw.error.log` - stderr
