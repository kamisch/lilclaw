...
async function runMatrixEntry(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lilclaw-ci-'));
  ...
  try {
    ...
    // Initialize lilclaw dir
    execSync('npx tsx -e "import { initLilclawDir } from \'./skills-engine/index.js\'; initLilclawDir();"', {
      cwd: tmpDir,
      stdio: 'pipe',
      timeout: 30_000,
    });
